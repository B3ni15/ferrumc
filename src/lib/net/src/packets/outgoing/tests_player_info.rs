...
mod tests {
    ...
    let add = PlayerWithActions::add_player(1234, "Alice", 0);
        assert_eq!(add.get_actions_mask(), 0x01);

    let remove = PlayerWithActions::remove_player(1234);
        assert_eq!(remove.get_actions_mask(), 0x02);

    let both = PlayerWithActions { uuid: 1234, actions: vec![PlayerAction::AddPlayer { name: "A".into(), properties: ferrumc_net_codec::net_types::length_prefixed_vec::LengthPrefixedVec::default(), gamemode: ferrumc_net_codec::net_types::var_int::VarInt::new(0), ping: ferrumc_net_codec::net_types::var_int::VarInt::new(0), display_name: None }, PlayerAction::RemovePlayer {} ] };
    ...
    let p = PlayerWithActions::add_player_with_properties(42, "Bob", 0);
